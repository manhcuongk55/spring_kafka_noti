...
public final class MultipleConsumersSendNotiService {
        ...
        private static final String topic = "cuong";
        private static final int numberOfConsumer = 100;
	...
	public String SendMess(String mess, int typeMess) throws JSONException {
		...
		if (messJson.has("appVersion")) {
			input.put("appVersion", messJson.get("appVersion"));
		}
		if (messJson.has("deviceVersion")) {
			input.put("deviceVersion", messJson.get("deviceVersion"));
		}
		...
		if(typeMess == 0){
			...
			NotificationProducerThread producerThread = new NotificationProducerThread(topic, typeMess,
					input, elasticsearchUtils, propPro);
			Thread t1 = new Thread(producerThread);
			t1.start();
		}
